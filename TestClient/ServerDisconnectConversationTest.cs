...
[TestClass]
public class ServerDisconnectConversationTest : BaseConversationTest
{
    [TestMethod]
    [DataRow(1.1d, wsHttp1Url)]
    [DataRow(2.0d, wsHttp2Url)]
    public async Task Should_run_WebSocket_conversation_successfully_where_server_disconnects(double httpVersion, string url)
    {
        // GIVEN
        ...
        var msgs = await new ServerDisconnectConversation(wsc).RunAsync(default);
        ...
        Assert.AreEqual("Disconnect!", msgs[2].ReadAsUtf8Text());
    }
}